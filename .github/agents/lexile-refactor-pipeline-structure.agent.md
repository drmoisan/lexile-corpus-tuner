# Agent Instructions: lexile-refactor-pipeline-structure (Issue #15, Parent #10)
...
Goal (from spec/plan):
- Move pipeline scripts from `scripts/production/` into `src/lexile_corpus_tuner/...` with proper package layout and imports.
...
Scope (do these):
- Create target package layout under `src/lexile_corpus_tuner/<pipeline_subdir>/` with `__init__.py` as needed.
...
- Optionally add short stubs in `scripts/production` that direct users to new commands if deprecations are needed.
