...
- [x] [P0-T1] Read repo Copilot instructions (policy precondition) (REQ-005)
  - File: `.github/copilot-instructions.md`
  - Acceptance: confirm the “no secrets” and “follow repo policies” constraints are understood.

- [x] [P0-T2] Read general code-change policy (policy precondition) (REQ-005)
...
- [x] [P0-T3] Read general unit-test policy (policy precondition) (REQ-005)
...
- [x] [P0-T4] Read Python code-change policy (policy precondition) (REQ-005)
...
- [x] [P0-T5] Read Python unit-test policy (policy precondition) (REQ-005)
...
- [x] [P0-T6] Capture current line anchors for the prompt literals that must change (REQ-001, REQ-002, REQ-003, REQ-004)
...
- [x] [P0-T7] Capture baseline unit test failures for prompt builder tests (REQ-005)
...
- [x] [P0-T8] Capture baseline QC statuses (Black, Ruff, Pyright, Pytest) (REQ-006)
...
- [x] [P1-T1] Add `PromptBuilderFileSystem` protocol in `scripts/dev_tools/atomic_executor/prompt_builder.py` (REQ-005)
...
- [x] [P1-T2] Add `RealPromptBuilderFileSystem` implementation in `scripts/dev_tools/atomic_executor/prompt_builder.py` (REQ-005)
...
- [x] [P1-T3] Add injectable `fs` parameter to `PromptBuilder.__init__` with default `RealPromptBuilderFileSystem()` (REQ-005)
...
- [x] [P1-T4] Add injectable `plan_resolver` parameter to `PromptBuilder.__init__` with default `resolve_feature_plan` (REQ-005)
  - Target symbol: `PromptBuilder.__init__`.
...
- [x] [P1-T5] Replace all direct filesystem reads/checks in `PromptBuilder` with `self._fs` calls (REQ-005)
  - Target symbols:
...
- [x] [P1-T6] Add `InMemoryPromptBuilderFileSystem` test helper in `tests/scripts/dev_tools/atomic_executor/test_prompt_builder.py` (REQ-005)
...
- [x] [P2-T1] Update `TestPromptBuilderInit.test_init_with_valid_template` to in-memory filesystem + injected `fs` (REQ-005)
...
- [x] [P2-T2] Update `TestPromptBuilderInit.test_init_raises_for_nonexistent_template` to in-memory (REQ-005)
...
- [x] [P2-T3] Update `TestPromptBuilderInit.test_init_raises_for_directory_template` to in-memory (REQ-005)
  - Required setup: `fs.dirs` contains the template path and `fs.files` does not.
  - Acceptance: `FileNotFoundError` is raised.

- [x] [P2-T4] Update `TestPromptBuilderBuild.test_build_combines_template_and_context` to in-memory + injected `plan_resolver` (REQ-005)
...
- [x] [P2-T5] Update `TestPromptBuilderBuild.test_build_handles_missing_user_story` to in-memory (REQ-005)
...
- [x] [P2-T6] Update `TestPromptBuilderBuild.test_build_raises_for_missing_plan` to in-memory (REQ-005)
...
- [x] [P2-T7] Update `TestPromptBuilderBuild.test_build_raises_for_missing_spec` to in-memory (REQ-005)
  - Required setup: omit spec.md path from `fs.files`.
  - Acceptance: `FileNotFoundError` matches "Missing required spec.md".

- [x] [P2-T8] Update `TestPromptBuilderBuild.test_build_injects_task_details` to in-memory (REQ-005)
  - Acceptance: prompt contains `- [P2-T5] Important task` and the CURRENT TASK header.

- [x] [P2-T9] Update `TestPromptBuilderBuild.test_build_includes_toolchain_instructions` to in-memory (REQ-005)
...
- [x] [P2-T10] Update `TestPromptBuilderBuild.test_build_includes_plan_update_instructions` to in-memory (REQ-005)
  - Acceptance: existing assertions still pass.

- [x] [P2-T11] Update `TestPromptBuilderEdgeCases.test_build_handles_empty_template` to in-memory (REQ-005)
  - Acceptance: existing assertions still pass.

- [x] [P2-T12] Update `TestPromptBuilderEdgeCases.test_build_handles_empty_plan` to in-memory (REQ-005)
  - Acceptance: existing assertions still pass.

- [x] [P2-T13] Update `TestPromptBuilderEdgeCases.test_build_uses_posix_paths_in_output` to in-memory (REQ-005)
  - Acceptance: existing assertions still pass.

- [x] [P2-T14] Update `TestPromptBuilderEdgeCases.test_read_text_helper_uses_utf8` to in-memory (REQ-005)
  - Acceptance: existing Unicode assertions still pass.

- [x] [P2-T15] Verify `test_prompt_builder.py` contains no `tmp_path` usage and no `.write_text()` calls (REQ-005)
...
- [x] [P3-T1] Update toolchain assertion test to require `python -m poetry run` forms and forbid `poetry run` forms (REQ-001)
...
- [x] [P3-T2] Add assertion that built prompt does not contain `/model` (REQ-002)
  - File: `tests/scripts/dev_tools/atomic_executor/test_prompt_builder.py`
...
- [x] [P3-T3] Add assertion that built prompt does not contain `"interactive session"` (REQ-003)
  - File: `tests/scripts/dev_tools/atomic_executor/test_prompt_builder.py`
...
- [x] [P3-T4] Add failing test for `<feature>` substitution when feature_dir is under `workspace / "docs" / "features" / "active"` (REQ-004)
  - File: `tests/scripts/dev_tools/atomic_executor/test_prompt_builder.py`
...
- [x] [P3-T5] Run prompt builder tests and confirm Phase 3 tests fail (REQ-001..REQ-004)
...
- [x] [P4-T1] Replace QC toolchain instruction lines in `PromptBuilder.build()` (REQ-001)
  ...
  - Acceptance: `[P3-T1]` passes and the substring `poetry run black` does not appear anywhere in the prompt.

- [x] [P4-T2] Remove interactive-only `/model` guidance from `PromptBuilder.build()` (REQ-002, REQ-003)
  - File: `scripts/dev_tools/atomic_executor/prompt_builder.py`
  ...
  - Acceptance: `[P3-T2]` and `[P3-T3]` pass.

- [x] [P4-T3] Implement deterministic `<feature>` placeholder substitution for feature folders under docs/features/active (REQ-004)
  - File: `scripts/dev_tools/atomic_executor/prompt_builder.py`
  ...
  - Acceptance: `[P3-T4]` passes.

---

**Phase 5 — Verify regressions + full toolchain loop (mandatory)**

- [x] [P5-T1] Re-run prompt builder tests and confirm all pass (REQ-001..REQ-005)
  - Command: `python -m poetry run pytest tests/scripts/dev_tools/atomic_executor/test_prompt_builder.py -q`
  - Acceptance: all tests pass.

- [x] [P5-T2] Run the full repo toolchain loop until one clean pass completes (REQ-006)
...
- [x] [P6-T1] Generate a real prompt via atomic executor `--print-prompt` and assert the fixed strings (REQ-001..REQ-004)
  - Command:
...
- [x] [P6-T2] Re-run a representative `execute-all` repro and assert no permission-denied substring appears (REQ-006)
...
- [x] [P7-T1] Run the Phase 6 repro once and branch deterministically on the result (REQ-007)
  - If `.agent_logs/` contains `Permission denied and could not request permission from user`, proceed to `[P7-T2]`.
  - If `.agent_logs/` contains zero matches, mark `[P7-T2]` and `[P7-T3]` as skipped with note: `SKIPPED: REQ-006 satisfied without allow-all-paths.`
  - **RESULT**: Latest log `atomic_executor_2026-01-15_221048.log` contains 0 matches. REQ-006 satisfied.

- [x] [P7-T2] Add `--allow-all-paths` to Copilot argv in `scripts/dev_tools/atomic_executor/cli.py` (REQ-007)
  - SKIPPED: REQ-006 satisfied without allow-all-paths.

- [x] [P7-T3] Re-run Phase 6 repro and assert substring is absent (REQ-007)
  - SKIPPED: REQ-006 satisfied without allow-all-paths.
...
- [x] [P8-T1] Update `docs/features/active/2026-01-11-copilot-cli-permission-failure-83/v2/spec.md` status and acceptance criteria evidence (REQ-001..REQ-007)
  - Required edits:
...
- [x] [P8-T2] Update `docs/features/active/2026-01-11-copilot-cli-permission-failure-83/v2/issue.md` with post-fix evidence excerpt (REQ-006)
