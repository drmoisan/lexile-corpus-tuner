...
- [ ] [P0-T1] Link approved spec for Issue #83 (REQ-01..REQ-05)
...
- [ ] [P0-T2] Record branch/commit baseline in log artifact (REQ-05)
	- Run:
...
- [ ] [P0-T3] Capture Copilot CLI runtime baseline (REQ-01)
	- Run:
		- `copilot --version`
...
- [ ] [P0-T4] Capture QC baseline without modifying files (REQ-05)
	- Run:
...
**Phase 1 — Preparation**
- [ ] [P1-T1] Confirm scope is locked for this fix (REQ-01..REQ-05)
...
- [ ] [P1-T2] Confirm target code locations and current line anchors (REQ-01, REQ-04)
...
- [ ] [P2-T1] Update regression test to require programmatic mode (`-p`) and forbid stdin-prompt feeding (REQ-01, REQ-04)
...
- [ ] [P2-T2] Add regression assertion that prompt file is still written to disk for `@path` expansion (REQ-02)
	- File: `tests/scripts/dev_tools/atomic_executor/test_cli.py`
...
- [ ] [P2-T3] Run the modified regression test and confirm it fails before the fix (REQ-04)
	- Run:
...
**Phase 3 — Minimal Fix**
- [ ] [P3-T1] Switch Copilot invocation to programmatic mode and reference the prompt file via `@path` (REQ-01, REQ-02)
...
- [ ] [P3-T2] Ensure required tool approvals remain explicit and include both Python command variants (REQ-03)
	- File: `scripts/dev_tools/atomic_executor/cli.py`
...
- [ ] [P3-T3] Add workspace directory allowance to reduce file-access prompts in headless runs (REQ-05)
	- File: `scripts/dev_tools/atomic_executor/cli.py`
	- Exact change:
...
- [ ] [P4-T1] Re-run the regression test and confirm it passes (REQ-01..REQ-04)
	- Run:
		- `poetry run pytest tests/scripts/dev_tools/atomic_executor/test_cli.py::TestRunCopilot::test_run_copilot_invokes_with_correct_arguments -q`
	- Acceptance: Test passes.
- [ ] [P4-T2] Run a local end-to-end repro of the original failing workflow (REQ-05)
...
- [ ] [P4-T3] Run the full toolchain loop and restart from formatting if any step fails/changes files (REQ-05)
...
- [ ] [P5-T1] Update Issue #83 documentation with final behavior and validation evidence (REQ-01..REQ-05)
	- Files:
...
**Phase 6 — PR & Handoff**
- [ ] [P6-T1] Prepare PR notes with deterministic verification checklist and risk statement (REQ-05)
	- Include:
